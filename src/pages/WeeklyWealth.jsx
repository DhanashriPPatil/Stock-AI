...
export const WeeklyWealth = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const res = await fetchParsedReport("weekly_wealth");
        if (res.exists) {
          setPdfData(res.data);
        }
      } catch (e) {
        console.error("Failed to load parsed WeeklyWealth intelligence:", e);
      }
    };
    fetchData();
  }, []);
  ...
  // --- DEFAULT MOCK DATA FROM THE PDF COPIED AS FALLBACK ---

  // Page 2: Nifty Outlook stats
  const defaultNiftyOutlook = {
    nifty: "24176",
    weeklyChg: "0.74%",
    trendStatus: "Uptrend",
    breadth: "Positive",
  ...
  const niftyOutlook = pdfData?.niftyOutlook || defaultNiftyOutlook;

  // Page 3: Trend Sector Rotation Data (for Relative Rotation Scatter Plot)
  const defaultSectorRotationData = [
  ...
  const sectorRotationData = pdfData?.sectorRotationData || defaultSectorRotationData;

  // Page 4: Market Breadth Historical records
  const defaultMarketBreadthData = {
    columns: [
      "Date",
      "10 DMA (Qty)",
      "20 DMA (Qty)",
  ...
  const defaultBseSectorPerformance = [
  ...
  const marketBreadthData = pdfData?.marketBreadthData || defaultMarketBreadthData;
  const optionsOiData = pdfData?.optionsOiData || defaultOptionsOiData;
  const bseSectorPerformance = pdfData?.bseSectorPerformance || defaultBseSectorPerformance;
  const topOiGainers = pdfData?.topOiGainers || defaultTopOiGainers;
  const topOiLosers = pdfData?.topOiLosers || defaultTopOiLosers;
  const belAnalysis = pdfData?.belAnalysis || defaultBelAnalysis;
    ...
    >
      ...
      <div className="flex flex-wrap items-center gap-2 border-b border-dash-line pb-0.5 font-mono">
        {[
          ...
          { id: "sectors", label: "Sector Rotations & OI", icon: BarChart3 },
      ...
      <div className="mt-4">
        ...
        {activeTab === "overview" && (
          ...
          >
            ...
            <div className="lg:col-span-3 space-y-6">
              <div className="glass-premium p-6 rounded-2xl space-y-4">
                <div className="flex items-center gap-2 pb-2 border-b border-dash-line">
                  <span className="w-2.5 h-2.5 rounded bg-dash-primary" />
                  <h3 className="text-xs font-black tracking-widest text-[#f8fafc] font-mono">
                    WEEK GONE SUMMARY
                  </h3>
                </div>
            ...
            <div className="lg:col-span-1 space-y-6">
              <div className="glass-premium p-5 rounded-2xl space-y-4 border border-dash-line shadow-lg font-mono">
                ...
                <div className="space-y-3.5">
                  <div className="bg-dash-bg/50 p-3 rounded-xl border border-dash-line flex justify-between items-center">
                    <span className="text-[10px] font-black text-dash-muted uppercase font-sans">
                      NIFTY 50 INDEX
                    </span>
                  ...
                  <div className="bg-dash-bg/50 p-3 rounded-xl border border-dash-line flex justify-between items-center">
                    ...
                    <span className="text-xs font-extrabold text-dash-success uppercase tracking-wider font-sans">
            ...
            <div className="glass-premium p-6 rounded-2xl space-y-4 font-mono">
              <div className="flex items-center justify-between border-b border-dash-line pb-4 flex-wrap gap-2">
                <div className="flex items-center gap-2">
                  ...
                  <h3 className="text-xs font-black tracking-widest text-[#f8fafc]">
                    RELATIVE ROTATION GRAPH (MARKET VECTOR)
                  </h3>
                </div>
                ...
                {marketBreadthData.segments.map((segment, segIdx) => (
                  <div key={segIdx} className="space-y-2">
                    <div className="flex items-center justify-between">
                      ...
                      <span className="text-[9px] font-semibold text-dash-muted">
                    ...
                    <div className="overflow-x-auto border border-dash-line rounded-xl">
                      <table className="w-full text-left font-mono text-[10px]">
                        <thead>
                          ...
                          <tr className="bg-dash-bg border-b border-dash-line text-xxs border-white/[0.04]">
        ...
        {activeTab === "technicals" && (
          <motion.div
            initial={{ opacity: 0, y: 10 }}
            animate={{ opacity: 1, y: 0 }}
            className="glass-premium p-6 rounded-2xl space-y-6 font-mono animate-fadeIn"
          >
            <div className="flex items-center justify-between border-b border-dash-line pb-4 flex-wrap gap-2">
              <div className="flex items-center gap-2">
                <LineChart className="w-5 h-5 text-dash-primary" />
                <h3 className="text-xs font-black tracking-widest text-[#f8fafc]">
                  NIFTY OPTION CHAIN INTENSITY (OI MULTIPLE LACs)
                </h3>
              </div>
              <span className="text-[8.5px] font-mono bg-dash-primary/10 border border-dash-primary/20 text-dash-primary px-2 py-0.5 rounded font-bold uppercase">
                Active Strike Open Interests
              </span>
            </div>
            ...
            <div className="h-96 w-full">
              <ResponsiveContainer width="100%" height="100%">
                ...
                >
                  ...
                  <YAxis stroke="#94a3b8" fontSize={10} unit="L" />
                  <Tooltip
                    contentStyle={{
                      background: "#070b19",
                  ...
                  />
                  <Bar dataKey="calls" name="Calls Open Interest" fill="#f43f5e" radius={[4, 4, 0, 0]} />
                  <Bar dataKey="puts" name="Puts Open Interest" fill="#10b981" radius={[4, 4, 0, 0]} />
                </BarChart>
              </ResponsiveContainer>
            </div>
          </motion.div>
        ...
        {/* TAB 4: SECTOR ROTATIONS & SUMMARY */}
        {activeTab === "sectors" && (
          <motion.div
            initial={{ opacity: 0, y: 10 }}
            animate={{ opacity: 1, y: 0 }}
          ...
          >
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 font-mono">
              {/* Gainers side */}
              <div className="glass-premium p-6 rounded-2xl space-y-4">
                <div className="flex justify-between items-center border-b border-dash-line pb-3">
                  <span className="text-xs font-black tracking-widest text-dash-success uppercase">
                    Weekly Top Open Interest Gainers
                  </span>
                  <span className="text-[9px] font-mono bg-dash-success/15 text-dash-success border border-dash-success/20 px-1.5 py-0.5 rounded leading-none">
                    Short Covering
                  </span>
                </div>
                <div className="overflow-x-auto border border-dash-line rounded-xl">
                  <table className="w-full text-left text-xs">
                    <thead>
                      <tr className="bg-dash-bg border-b border-dash-line font-bold text-dash-muted">
                        <th className="p-3 uppercase">Scrip</th>
                        <th className="p-3 text-right uppercase">Price</th>
                        <th className="p-3 text-right uppercase text-dash-success">Price Chg</th>
                        <th className="p-3 text-right uppercase">OI</th>
                        <th className="p-3 text-right uppercase text-dash-success">OI Chg</th>
                      </tr>
                    </thead>
                    <tbody>
                      {topOiGainers.map((gain, idx) => (
                        <tr
                          key={idx}
                          className="border-b border-dash-line hover:bg-white/[0.02] transition-colors"
                        >
                          ...
                          <td className="p-3 text-right text-[#f1f5f9] font-bold">
                            ₹{gain.price}
                          </td>
                          <td
                            className={`p-3 text-right font-black ${gain.priceChg >= 0 ? "text-dash-success" : "text-dash-danger"}`}
                          >
                            {gain.priceChg >= 0 ? `+${gain.priceChg}%` : `${gain.priceChg}%`}
                          </td>
              ...
              <div className="glass-premium p-6 rounded-2xl space-y-4">
                <div className="flex justify-between items-center border-b border-dash-line pb-3">
                  <span className="text-xs font-black tracking-widest text-dash-danger uppercase">
                    Weekly Top Open Interest Losers
                  </span>
                  <span className="text-[9px] font-mono bg-dash-danger/15 text-dash-danger border border-dash-danger/20 px-1.5 py-0.5 rounded leading-none">
                    Long Unwinding
                  </span>
                </div>
                <div className="overflow-x-auto border border-dash-line rounded-xl">
                  <table className="w-full text-left text-xs">
                    <thead>
                      <tr className="bg-dash-bg border-b border-dash-line font-bold text-dash-muted">
                        ...
                        <th className="p-3 text-right uppercase text-dash-success">Price Chg</th>
                        <th className="p-3 text-right uppercase">OI</th>
                        <th className="p-3 text-right uppercase text-dash-danger">OI Chg</th>
                      </tr>
                    </thead>
                    <tbody>
                      {topOiLosers.map((lose, idx) => (
                        <tr
                          key={idx}
                          className="border-b border-dash-line hover:bg-white/[0.02] transition-colors"
                        >
                          ...
                          <td
                            className={`p-3 text-right font-black ${lose.priceChg >= 0 ? "text-dash-success" : "text-dash-danger"}`}
                          >
                            {lose.priceChg >= 0 ? `+${lose.priceChg}%` : `${lose.priceChg}%`}
                          </td>
                          <td className="p-3 text-right text-dash-muted">
                            {lose.oi}
                          </td>
          ...
          >
            ...
            <div className="grid grid-cols-1 xl:grid-cols-3 gap-6 font-mono">
              ...
              <div className="space-y-6">
                ...
                <div className="glass-premium p-5 rounded-2xl space-y-3.5 border border-dash-line font-mono">
                  <span className="text-xs font-black tracking-widest text-white font-sans block border-b border-dash-line pb-3 uppercase animate-pulse">
                    FII & DII In-and-Outflow (INR Cr.)
                  </span>
                  <div className="grid grid-cols-2 gap-4">
                    <div className="space-y-2">
                      <span className="text-[10px] text-dash-success font-black block">
                        DII NET (Domestic)
                      </span>
                    ...
                    <div className="space-y-2">
                      <span className="text-[10px] text-dash-danger font-black block">
                        FII NET (Foreign)
                      </span>
                      <div className="bg-dash-bg/40 border border-dash-line p-2.5 rounded-lg text-center font-mono">
                        <span className="text-[9px] text-dash-muted block">
                  ...
                  <table className="w-full text-left font-mono text-xs">
                    <thead>
                      <tr className="bg-dash-bg border-b border-dash-line font-bold text-dash-muted">
                        ...
                        <th className="p-3 text-right uppercase font-sans">
                          Index Price (Rs.)
                        </th>
              ...
              <div className="glass-premium p-6 rounded-2xl space-y-4">
                ...
                <div className="overflow-x-auto border border-dash-line rounded-xl">
                  <table className="w-full text-left font-mono text-xs">
                    <thead>
                      <tr className="bg-dash-bg border-b border-dash-line font-bold text-dash-muted">
                        <th className="p-3 uppercase font-sans">Scrip / Stock Name</th>
                        <th className="p-3 text-right uppercase">
                          Index Price (Rs.)
                        </th>
                        <th className="p-3 text-right uppercase text-dash-danger font-sans">
                          Decline Wave
                        </th>
                      </tr>
                    </thead>
        ...
        {activeTab === "featured" && (
          <motion.div
            initial={{ opacity: 0, y: 10 }}
            animate={{ opacity: 1, y: 0 }}
            className="grid grid-cols-1 lg:grid-cols-4 gap-6 items-start animate-fadeIn"
          >
            ...
            <div className="lg:col-span-3 space-y-6">
              <div className="glass-premium p-6 rounded-2xl space-y-5">
                ...
                <div className="space-y-4">
                  <span className="text-xs font-black tracking-widest text-[#00c2ff] block font-mono uppercase">
            ...
            <div className="lg:col-span-1 space-y-6">
              ...
              <div className="glass-premium p-5 rounded-2xl border border-dash-line space-y-4">
                <span className="text-xs font-black tracking-widest text-white font-mono block border-b border-dash-line pb-3 uppercase">
                  Execution Metrics
                </span>

                <div className="space-y-3 text-xs">
                  <div className="flex justify-between items-center p-2.5 bg-dash-bg/50 border border-dash-line/40 rounded-xl font-mono">
                    <span className="text-[10px] text-dash-muted font-sans">
                      TARGET HIGH
                    </span>
                  ...
                  <div className="flex justify-between items-center p-2.5 bg-dash-bg/50 border border-dash-line/40 rounded-xl font-mono">
                    ...
                    <span className="font-light font-bold text-dash-success">
                  ...
                  <div className="flex justify-between items-center p-2.5 bg-dash-bg/50 border border-dash-line/40 rounded-xl font-mono">
                    ...
                    <span className="font-black text-dash-danger">
              ...
              <div className="glass-premium p-5 rounded-2xl border border-dash-line space-y-4">
                ...
                <div className="space-y-2.5 text-xs">
                  {belAnalysis.executionData.indicators.map((ind, idx) => (
                    ...
                    >
                      <span className="text-dash-muted font-sans">{ind.name}</span>
                      <span
              ...
              <div className="glass-premium p-5 rounded-2xl border border-dash-line space-y-4">
                ...
                <div className="space-y-3 text-xs">
                  <div className="flex justify-between items-center font-mono">
                    <span className="text-dash-muted text-[10px] font-sans">
                      Benchmark Nifty
                    </span>
                  ...
                  <div className="flex justify-between items-center font-mono">
                    ...
                    <span className="text-dash-success font-black text-emerald-400">
      ...
      >
        <div className="flex items-center gap-2 border-b border-white/[0.05] pb-2 font-mono">
          <FileText className="w-4 h-4 text-dash-muted" />
          <span className="text-[10px] font-black tracking-wider text-dash-muted uppercase">
