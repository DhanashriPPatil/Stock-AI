...
export const SectorUpdate = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const res = await fetchParsedReport("sector_update");
        if (res.exists) {
          setPdfData(res.data);
        }
      } catch (e) {
        console.error("Failed loading parsed sector update report:", e);
      }
    };
    fetchData();
  }, []);
  ...
  const defaultCommodityPriceData = [
  ...
  const commodityPriceData = pdfData?.commodityPriceData || defaultCommodityPriceData;
  const itCompanies = pdfData?.itCompanies || defaultItCompanies;
