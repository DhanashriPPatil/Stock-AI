...
export const processPdfFolder = async () => {
  const response = await API.post("/process-pdf-folder");
  return response.data;
};

export const fetchParsedReport = async (type) => {
  const response = await API.get(`/parsed-report/${type}`);
  return response.data;
};
